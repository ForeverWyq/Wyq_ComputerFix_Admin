      ...
      <el-table :data="waiterObj.list" stripe style="width: 100%">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <span class="setSapn" @click="deleteWaiterById(scope.row.id)"><i class="el-icon-delete"></i></span>
						<!-- scope.row 点击哪一行的编辑按钮 它就是哪一行的一个对象 -->
            <span class="setSapn" @click="editWaiter(scope.row)"><i class="el-icon-edit"></i></span>
            <span class="setSapn" @click="toWaiterDetail(scope.row.id)">详情</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    // 辅助函数 来获取actions中的方法
    // 第一个参数 命名空间对应的名称
    ...mapActions("waiter", ["findAllWaiter", "deleteWaiter","saveOrUpdateWaiter"]),
    ...
    deleteWaiterById(id) {
      this.$confirm("此操作将永久删除该员工信息, 是否继续?", "提示", {
        ...
        .then(() => {
          // 点击确定的时候触发
          // 执行删除操作
          this.deleteWaiter(id).then((r) => {
            // 对页数进行处理 Math.ceil向上取整 1.4 -》 2
            const totalPage = Math.ceil(
              (this.waiterObj.total - 1) / this.pageSize
            );
            this.currentPage =
		...
		saveOrUpdate(ruleFormRef){
			...
			this.$refs[ruleFormRef].validate((valid) => {
				if (valid) {
					// 验证通过时
					// 发送网络请求
					this.saveOrUpdateWaiter(this.ruleForm)
					.then(r => {
						// 提示是否成功
						let messageInfo = ''
						// 新增是没有id的 编辑是有id的
